...
class Model_orders extends CI_Model
{
	...
	public function getOrdersData($id = null)
	{
		...
		if($id) {
			...
			WHERE orders.user_id= ? AND orders.o_id = ?";
	...
	{	$user_id = $this->session->userdata('id');
		if($date) {
			...
			WHERE orders.paid_status = ? and orders.date_sold = ? and orders.user_id =?
			ORDER BY orders.o_id DESC";
			$query = $this->db->query($sql, array(1, $selected_date, $user_id));
	...
	{
		$user_id = $this->session->userdata('id');
		$bill_no = 'DVG-'.strtoupper(substr(md5(uniqid(mt_rand(), true)), 0, 6));
    	$data = array(
    		'bill_no' => $bill_no,

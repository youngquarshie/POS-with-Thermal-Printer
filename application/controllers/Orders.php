...
class Orders extends Admin_Controller 
{
	...
	*/
	...
	function getCategoryProductBySearch(){
		...
		if($category_id && $product_name) {
			$product_data = $this->model_products->getProductDatabyCategoryandSearch($category_id, $product_name);
			if(!empty($product_data)){
				foreach($product_data as $product){
					$name=$product['name'];
					$price=$product['price'];
	...
	*/
	...
	public function printDiv($id)
	{
		...
		$printer -> text("055 707 9977.\n");
		$printer -> text("024 342 3842.\n");
		$printer -> text("020 812 5522.\n");
		$printer -> text("--------------------------------------.\n");
		...
		// $printer -> text("Username:".$userdata[0]['firstname']." ".$userdata[0]['lastname']."\n");
		$printer -> text("------------------------------------------"."\n");
		...
		$printer -> text("\n");
		$printer -> text("--------------------------------------.\n");
		$total= sprintf('%-30.30s%8.2f', "Total" . "", $order_data['gross_amount']."\n");
		$printer -> text($total);
		$printer -> setEmphasis(false);
		$printer -> feed();
		...
		$printer -> pulse();
				...
				/* Close printer */
				$printer -> close();
				$this->render_template('orders/index');
